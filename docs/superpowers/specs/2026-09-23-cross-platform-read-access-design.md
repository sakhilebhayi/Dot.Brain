...
Validation is defensive, mirroring `health.js:54-56`: a response is rejected as a contract failure (never partially trusted) if `contract` is not exactly `dot-revenue/v1`, if `generated_at` is missing or does not parse as a valid date, if `signals` is not an array, if `classification` is missing or not one of the four known tiers, or if any signal entry lacks a well-typed `key`/`value`. The contract exposes pre-aggregated business signals a platform itself computes and chooses to reveal — never raw transaction or PII-level records — mirroring Guardian's own principle of exposing operational signals, not internals (`health.js:5,13-14`). This is enforced structurally, not just by convention: a successful poll's `signals` are passed through `sanitizeSignal`, which keeps only the six documented fields (`key`/`value`/`unit`/`period`/`trend`/`confidence`) and drops anything else a response happened to include, so a platform can't smuggle extra fields across the boundary just by putting them in the JSON body. This is what keeps a multi-tenant platform in control of what crosses the boundary, satisfying the cross-tenant confidentiality rule ("cross-tenant inference prohibited except explicit sharing policies," `brain.governance.md:101`): the platform's own code decides what a signal aggregates over, not Brain.
...
A response whose declared `classification` exceeds the enrolling platform's own `classification_ceiling` is rejected outright — a safety rail beyond what Guardian needed, since a classification error on financial data is higher-stakes than one on a health check. The rejection ships today as a structured result returned to the caller (see Error handling); recording it as a standing, escalatable incident is a deferred extension, not yet designed (ADR-0018's Open questions).
...
- Malformed response (a non-JSON body, a wrong/missing `contract` discriminator, missing/invalid `generated_at`, missing/invalid `classification`, non-array `signals`, a signal missing/null/wrong-typed `key`/`value`): rejected as a contract failure, the body is never partially trusted, mirroring `health.js:54-56`. A well-formed signal's extra fields beyond the documented six are silently dropped by `sanitizeSignal`, not rejected outright -- they simply never cross the boundary.
- A manifest whose `signals_url` is not `https://`: rejected at enrollment time (`validateManifest`), before any poll is ever attempted against it.
...
| 1.0.3 | 2026-09-23 | Post-merge repeat-check, round 3 | §4's own sentence still said "raised as an incident" — round 2's fix corrected the Error handling bullet but missed this one; both now agree. `validateSignalsResponse` now type-checks `key` (string) and `value` (finite number), not just presence — a boolean or array previously passed. `validateManifest` now requires `platform`/`signals_url`/`token_env` to be non-empty strings (not just truthy), rejects unrecognized/miscased manifest keys instead of silently discarding them, and requires `poll_interval_s`/`timeout_ms` to be positive numbers — a bad `timeout_ms` used to surface as a misleading `kind: 'network'` failure. |
| 1.0.4 | 2026-09-23 | PR review (GitHub Copilot) | Three more findings from external PR review, all fixed: (1) `validateSignalsResponse` now rejects any response whose `contract` field isn't exactly `dot-revenue/v1`, so a future `dot-revenue/v2` or an unrelated response can no longer be silently accepted and fed downstream as v1. (2) A signal's extra fields beyond the six documented in §1 (`key`/`value`/`unit`/`period`/`trend`/`confidence`) are now stripped by a new `sanitizeSignal` before crossing into Brain's process — previously `pollPlatform` passed `body.signals` through unmodified, so a platform could smuggle arbitrary extra fields (including raw transaction or PII-level data) simply by including them in the response, contradicting §1's own "platforms expose signals, never raw records" principle. (3) `validateManifest` now requires `signals_url` to start with `https://` — this capability authenticates with a bearer token over a financial-data endpoint, and plaintext HTTP would send both in the clear. |
